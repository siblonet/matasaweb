async function ChangeSlade() {
    ...
    if (items && items.length > 0) {
        ...
        $("#logointerne").attr("src", `${items.find(item => item.which === "logointern") ? items.find(item => item.which === "logointern").image : "../assets/img/matasa.png"}`);
...
const SladeLoading = (which) => {
    if (which === "baVdeo") {
        ...
        $(`#${which}`).css("background-image", "url(adminpcdasborad/assets/img/loadingc.gif)");
...
async function sendAnnonce(base64Data, fileName, which, anonceid) {
    const response = await fetch(apiUrlfine + `boutique/annonce/matasa/${anonceid}`, {
        method: 'POST',
        headers: {
